...
class DashboardView:
    ...
    def view_request_log(self):
        response = requests.get("http://localhost:5000/api/requests/log")
        if response.status_code == 200:
    ...
    def search_assets(self):
        ...
        response = requests.get(f"http://localhost:5000/api/assets/search?q={search_query}")
        if response.status_code == 200:
            asset_data = response.json()
...
class LoginView:
    def __init__(self,master):
        self.master = master
        self.master.title("LoginPage")
        self.frame = tk.Frame(self.master)
        self.frame.pack(padx=600, pady=200, fill=tk.BOTH, expand=True)
        ...
        self.login_button = tk.Button(master, text="LOGIN",command=self.checkCredentials)
    ...
    def checkCredentials(self):
        ...
        response = requests.get(f"http://localhost:5000/api/? auth={username,password}")
        response.status_code=200
        if response.status_code ==200:
            self.master.destroy()
            dash = tk.Tk()
            dashboardPage = DashboardView(dash)
            dashboardPage.dash.mainloop()
        else:
            messagebox.showerror("Error","Invalid User")

if __name__ == "__main__":
    root = tk.Tk()
    loginPage = LoginView(root)
    root.mainloop()
